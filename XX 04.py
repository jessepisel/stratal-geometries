...
tops_api = pd.read_csv(
    r"F:\Geology\WSGS\Projects\jupyter\EarlyWSGS\ftunion.csv"
).fillna(
    0
...
plt.scatter(
    ...
    lancer["y_locs"],
    alpha=0.8,
    c=abs(laDF.diff(axis=1)).sum(axis=1),
    s=abs(laDF.diff(axis=1)).sum(axis=1) * 10,
    cmap="magma",
)
plt.colorbar()
...
plt.scatter(
    ...
    ftunion["y_locs"],
    alpha=0.8,
    c=abs(ftDF.diff(axis=1)).sum(axis=1),
    s=abs(ftDF.diff(axis=1)).sum(axis=1) * 10,
    cmap="magma",
)
plt.colorbar()

# +
fthoriz = ftunion[(ftunion.horiz_prob > 0.0)]

plt.figure(figsize=(30, 10))
plt.subplot(121)
plt.scatter(ftunion["x_locs"], ftunion["y_locs"], alpha=0.1)
plt.scatter(
    ftunion["x_locs"],
    ftunion["y_locs"],
    c=ftunion["trunc_prob"] * -1,
    cmap=truncCmap,
    vmin=-1,
    vmax=0,
)
...
lahoriz = lancer[(lancer.horiz_prob > 0.0)]

plt.subplot(122)
plt.scatter(
    lancer["x_locs"],
    lancer["y_locs"],
    c=lancer["trunc_prob"] * -1,
    cmap=truncCmap,
    vmin=-1,
    vmax=0,
)
